                ...
                @foreach($notifikasis as $notif)
                  @if($notif->status == "belum dibaca")
                    <tr style="background-color: #f4f6f9">
                      <td>{{$notif->kategori}}</td>
                      <td>{{$notif->deskripsi}}</td>
                      <td>{{$notif->created_at}}</td>
                      <td>
                        <a href="/kolektor/lihat_notifikasi/{{$notif->kategori}}/{{$notif->id}}" class="btn btn-primary"><abbr title="Lihat"><i class="fa fa-eye"></i> </abbr></a>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <div class="modal-footer justify-content-between">
              <form name="form" method="post" action="/kolektor/hapus_notifikasi">
                {{csrf_field()}}
